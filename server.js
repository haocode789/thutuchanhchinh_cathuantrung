...
async function initDB() {
    try {
        ...
        // Tự động kiểm tra và bổ sung cột 'id' nếu bảng cũ thiếu
        try {
            await db.execute(`SELECT id FROM procedures LIMIT 1`);
        } catch (e) {
            console.log('⚠️ Bảng procedures thiếu cột id, đang tiến hành nâng cấp cấu trúc...');
            await db.execute(`ALTER TABLE procedures ADD COLUMN id INTEGER PRIMARY KEY AUTOINCREMENT`);
        }
